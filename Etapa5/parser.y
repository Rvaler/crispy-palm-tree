...
type	: KW_FLOAT  	{ $$ = astCreate(AST_FLOAT, getLineNumber(), 0, 0, 0, 0, 0); }
		| KW_INT      	{ $$ = astCreate(AST_INT, getLineNumber(), 0, 0, 0, 0, 0); }
		| KW_CHAR     	{ $$ = astCreate(AST_CHAR, getLineNumber(), 0, 0, 0, 0, 0); }
		| KW_BOOL	    { $$ = astCreate(AST_BOOL, getLineNumber(), 0, 0, 0, 0, 0); }
		;
...
function: type TK_IDENTIFIER '(' arguments_list ')' command 	{ $$ = astCreate(AST_FUNCTION, getLineNumber(), $2, $1, $4, $6, 0); }
		| type TK_IDENTIFIER '(' ')' command                      { $$ = astCreate(AST_FUNCTION, getLineNumber(), $2, $1, 0, $5, 0); }
		;
...
if_command	: KW_IF '(' expression ')' KW_THEN command  { $$ = astCreate(AST_IF, getLineNumber(), NULL, $3, $6, 0, 0); }
			| KW_IF '(' expression ')' KW_THEN command KW_ELSE command   { $$ = astCreate(AST_IF_ELSE, getLineNumber(), NULL, $3, $6, $8, 0); }
			;
