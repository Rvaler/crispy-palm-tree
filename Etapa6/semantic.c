...
void setDataType(AST_NODE *node){
	switch(node->sons[0]->type) {
		...
		case AST_INT:
		// printf("type node %s %i\n", node->symbol->text, node->symbol->dataType);
		// 	// printf("type node %s %i, type node 2 %s %i\n", node->symbol->text, node->symbol->dataType,
		// 		// node->sons[0]->symbol->text, node->sons[0]->dataType);
...
//	{
//		if(declaredFunctions[i]->symbol->text == funcall_node->symbol->text){
//
//			AST_NODE *node = declaredFunctions[i];
//			return node;
//		}
//	}
	return 0;
...
//	for (i = 0; i < functionsCount; ++i)
//	{
//		AST_NODE *function = declaredFunctions[i];
//		printf("Function %i has type %i and name %s\n", i, function->dataType, function->symbol->text);
//
//		AST_NODE *node;
//		if((node = function->sons[1])){
//			while(node){
//				printf("First argument has type %i and name %s\n", node->sons[0]->dataType, node->sons[0]->symbol->text);
//				node = node->sons[1];
//			}
//		}
//	}
...
//			declaredFunctions[functionsCount] = node;
//			functionsCount++;
//		}
...
void checkDataTypes(AST_NODE *node){
	...
	case AST_PARENTHESES:
		...
		break;
	case AST_ATTR:
        if(node->sons[0]->symbol != 0) {
            if(!(verifyAttributionTypes(node->symbol->dataType, node->sons[0]->symbol->dataType))){
